...
bd_process <- function(lambda, mu, initial_state = 0, steps = 100) {
    ...
    for (i in 1:steps) {
        ...
        # Om tillståndet är 3 så är lambda 0
        if (state_now == 3) {
            lambda_now <- 0
        } else {
            lambda_now <- lambda
        ...
        # Om tillståndet är 0 så är mu 0
        if (state_now == 0) {
            mu_now <- 0
        } else {
            mu_now <- mu
        ...
        # Om tillståndet är 0 så minskar det med 1, annars så ökar det med 1
        if (state_now == 0) {
...
# Skapa en trappstegsplot av processen
plot(stepfun(time1[-1], state1),
     do.points = FALSE,         # Visa inte punkter
     xlab = "Tid (timmar)",     # X-axeletikett
     ylab = "Antal jobb",       # Y-axeletikett
     main = "Förgreningsprocess med lambda=1, mu=5", # Titel
     yaxt = "n")                # Inaktivera standard y-axel

# Lägg till anpassad y-axel med heltal
axis(2, at = c(0:3), las = 2)   # Horisontella etiketter
```
