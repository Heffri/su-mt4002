...
# För att jämföra med den stationära fördelningen (pi_stationary) säkerställer vi att båda vektorerna har samma ordning (tillstånd 0-5)
states <- 0:5
...
```{r}
# Hitta alla index i chain där maskinen är i tillstånd 5
indices <- which(chain == 5)
# Uteslut dag 1 (för vilken vi inte har någon föregående dag)
indices <- indices[indices > 1]

# Räkna antalet gånger där dagen innan var tillstånd 1
count_prev1 <- sum(chain[indices - 1] == 1)

# Beräkna den empiriska sannolikheten:
empirical_prob <- count_prev1 / length(indices)
...
- $\pi_1 = \text{pi_stationary}[2]$
- $\pi_5 = \text{pi_stationary}[6]$
