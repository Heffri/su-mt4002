...
simulate_markov_chain <- function(P, start_state, n_days) {
  n_states <- nrow(P)  # Antal tillstånd
  states <- numeric(n_days)  # Vektor för att spara tillstånden
  current_state <- start_state  # Starta i det angivna tillståndet
  
  for (day in 1:n_days) {
    states[day] <- current_state  # Spara aktuellt tillstånd
    current_state <- sample(1:n_states, size = 1, prob = P[current_state, ])  # Simulera övergång
  }
